...
# Light SaaS UI · native Streamlit sidebar · compact dashboard
...
/* compact Streamlit shell */
.main .block-container {{ padding-top:.55rem !important; padding-bottom:1.5rem !important; max-width:1380px;
    margin-left:auto !important; margin-right:auto !important; padding-left:1.1rem !important; padding-right:1.1rem !important; }}
#MainMenu, footer {{ visibility:hidden; }}
header[data-testid="stHeader"] {{ display:none; }}
[data-testid="collapsedControl"] {{ display:none; }}
section[data-testid="stSidebar"] {{ background:{CARD}; border-right:1px solid {BORDER}; width:255px !important; min-width:255px !important; }}
section[data-testid="stSidebar"] > div:first-child {{ padding-top:.85rem !important; padding-left:1rem !important; padding-right:1rem !important; }}
section[data-testid="stSidebar"] .stRadio > label p {{ font-size:11px; color:{MUTED}; letter-spacing:1.6px; text-transform:uppercase; font-weight:800; }}
section[data-testid="stSidebar"] [role="radiogroup"] label {{ padding:8px 10px; border-radius:10px; margin:3px 0; }}
section[data-testid="stSidebar"] [role="radiogroup"] label:hover {{ background:#F2F7F4; }}
section[data-testid="stSidebar"] [role="radiogroup"] label[data-baseweb="radio"] p {{ font-weight:700; color:{INK}; }}
...
.h-title {{ font-size:23px; font-weight:800; letter-spacing:-.5px; color:{INK}; margin:0 0 6px; }}
.h-desc {{ font-size:13px; color:{MUTED}; margin-bottom:8px; max-width:820px; line-height:1.55; }}
...
.hero {{ background:{CARD}; border:1px solid {BORDER}; border-radius:16px; padding:12px 14px; margin-bottom:10px;
        box-shadow:0 1px 3px rgba(16,42,34,.05); }}
.agent-card {{ background:{CARD}; border:1px solid {BORDER}; border-radius:16px; padding:11px 14px; margin:0 0 12px;
        box-shadow:0 1px 3px rgba(16,42,34,.05); }}
.agent-head {{ display:flex; justify-content:space-between; align-items:center; gap:10px; margin-bottom:8px; }}
.agent-title {{ font-size:15px; font-weight:800; color:{INK}; }}
.agent-status {{ font-size:11px; color:{ACCENT}; font-weight:700; background:{ACC_SOFT}; border:1px solid #BFE3D2; padding:4px 8px; border-radius:999px; }}
.timeline {{ position:relative; display:grid; grid-template-columns:repeat(6,1fr); gap:8px; align-items:start; }}
.timeline:before {{ content:""; position:absolute; left:6%; right:6%; top:18px; height:3px; background:#DCEAE3; border-radius:99px; }}
.timeline:after {{ content:""; position:absolute; left:6%; top:18px; height:3px; width:var(--fill,0%); background:{ACCENT}; border-radius:99px; transition:width .45s ease; }}
.titem {{ position:relative; z-index:1; text-align:center; min-width:0; }}
.tcircle {{ width:36px; height:36px; border-radius:999px; margin:0 auto 5px; display:flex; align-items:center; justify-content:center;
          background:#F7FAF8; border:2px solid #DCEAE3; color:{MUTED}; font-weight:800; box-shadow:0 0 0 5px {CARD}; }}
...
# ================================================================
with st.sidebar:
    st.markdown('<div class="brandbar" style="margin-bottom:22px;"><div class="blogo">L</div>'
                '<div><div class="t">Loan Approval<br>Intelligence</div>'
    ...
    st.markdown('<div class="cs" style="margin-top:24px;line-height:1.45;">GA-optimised fuzzy risk engine with integrated agentic evaluation.</div>',
                unsafe_allow_html=True)
