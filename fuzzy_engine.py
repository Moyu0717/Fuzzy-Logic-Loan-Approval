...
np.random.seed(0)
ref_idx = np.arange(len(loan_status))
...
print(f"\nReference sample: {len(ref_idx)} rows (FULL DATASET) "
...
def fuzzy_risk_score(inc_val, rat_val, emp_val,
    ...
    Used by the GUI for single-applicant evaluation.
    """
...
print("\n" + "─" * 60)
print("  Running GA MF Optimisation...")
print(f"  Population={GA_POP}  Generations={GA_GENS}")
print(f"  Crossover rate={GA_CR}  (Arslan & Kaya: prob=1)")
print(f"  Adaptive mutation  (Arslan & Kaya: condition-based)")
print(f"  Two-point crossover  (Arslan & Kaya: two-point)")
print(f"  Fitness = Σ(actual_status - fuzzy_risk)²")
print("─" * 60)
